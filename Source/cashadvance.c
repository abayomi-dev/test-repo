...
void SetEntryModeCA(const TRAN_LOG *pstLog)
{
	...
	if( ChkIfAmex() )
	{
		...
		SetAmexEntryModeCA(pstLog);
	}
	else
	{
		//ShowLogs(1, "Inside SetEntryMode 2");
		SetStdEntryModeCA(pstLog);
	}
}

void SetAmexEntryModeCA(const TRAN_LOG *pstLog)
{
...
void SetStdEntryModeCA(const TRAN_LOG *pstLog)
{
	//------------------------------------------------------------------------------
	// Entry mode digit 1
	#ifdef ENABLE_EMV
...
void SetCommReqFieldCA(void)
{
	...
	//Start here	
	SetEntryModeCA(&glProcInfo.stTranLog);		// bit 22, entry mode
	SetCondCodeCA();		// bit 25, service condition code 
...
int CashAdvanceProcICCMsg(void)
{
	int		iRet;
...
int cashAdvance()
{
	...
	while(1)
	{
		...
		{
			txnType = 2;
			UtilPutEnv("actype", "01");//Cash Advance
			iRet = CashAdvanceProcICCMsg();
