...
void SetEntryModeRf(const TRAN_LOG *pstLog)
{
	...
	if( ChkIfAmex() )
	{
		...
		SetAmexEntryModeRf(pstLog);
	}
	else
	{
		//ShowLogs(1, "Inside SetEntryMode 2");
		SetStdEntryModeRf(pstLog);
	}
}

void SetAmexEntryModeRf(const TRAN_LOG *pstLog)
{
...
void SetStdEntryModeRf(const TRAN_LOG *pstLog)
{
	//------------------------------------------------------------------------------
	// Entry mode digit 1
	#ifdef ENABLE_EMV
...
void SetCommReqFieldRf(void)
{
	...
	//Start here	
	SetEntryModeRf(&glProcInfo.stTranLog);		// bit 22, entry mode
	SetCondCodeRf();		// bit 25, service condition code 
...
int refundTxn()
{
	...
	while(1)
	{
		...
		{
			txnType = 4;
			UtilPutEnv("actype", "20");//Refund
			iRet = RefundProcICCMsg();
