...
int CustomizeAppLibForArabiaLang(uchar bSetToArabia)
{
    ...
    if (bSetToArabia)
    {
		...
		Gui_Init(_RGB_INT_(207,53,46), _RGB_INT_(26,13,171), "PAXARFA.FONT");
...
{
	ShowLogs(1, "Initiate transaction parameters");
	//memset(&glProcInfo, 0, sizeof(SYS_PROC_INFO));//Commented out by Wisdom
...
void PromptRemoveICC(void)
{
	...
	{
		Gui_ClearScr();//Wisdom
		ShowLogs(1, "PromptRemoveICC: Step 1");
		return;
	}
	...
	{
		Gui_ClearScr();//Wisdom
		ShowLogs(1, "PromptRemoveICC: Step 2");
		// 如果IC卡已拔出，直接返回。
	...
	//GetTransName(szName);
	displayName(szName);
	Gui_ShowMsgBox(szName, gl_stTitleAttr, _T("REMOVE CARD"), gl_stCenterAttr, GUI_BUTTON_NONE, 0, NULL);
	...
	ShowLogs(1, "PromptRemoveICC: Step 3");
}
...
void EtopLogo(void)
{
	...
	/*uchar	*psLogoData;
	int		iWidth, iHeigh;
	ShowLogs(1, "Getting Display Logo");
	psLogoData = NULL;
	Gui_ClearScr();
	...
	if (psLogoData!=NULL)
	{
		...
		ShowLogs(1, "Height: %d. Width: %d", iHeigh, iWidth);
...
#ifdef APP_DEBUG
	UtilGetEnvEx("bankremarks", bankAbbr);
	Gui_DrawText(bankAbbr, stTextAttr, 0, 35);
#endif

#if defined(EMV_TEST_VERSION) && defined(ENABLE_EMV)
	Gui_DrawText("UP", stTextAttr, 0, 50);
#endif

	if (ChkIfTrainMode())
	{
		Gui_DrawText("DEMO", stTextAttr, 0, 65);
	}
}
...
void DispBlockFunc(void)
{
	Gui_ClearScr();
	PubBeepErr();
	Gui_ShowMsgBox(GetCurrTitle(), gl_stTitleAttr, _T("Transaction Terminated"), gl_stCenterAttr, GUI_BUTTON_CANCEL, 3, NULL);
}

void DispProcess(void)
{
	Gui_ClearScr();
	Gui_ShowMsgBox(GetCurrTitle(), gl_stTitleAttr, _T("PROCESSING CARD"), gl_stCenterAttr, GUI_BUTTON_NONE, 0, NULL);
}
...
//	sprintf(stBuff[nCnt].szLine, "%s:%.8s", _T("TERM ID"), glSysParam.stEdcInfo.szDownLoadTID);
	//Modified by Kevin Liu 20160621 bug839
	sprintf(stBuff[nCnt].szLine, "%s:%.8s", _T("DOW TID"), glSysParam.stEdcInfo.szDownLoadTID);
	stBuff[nCnt++].stLineAttr = gl_stLeftAttr;
...
void GetTransName(unsigned char *szName)
{
	...
	{
		//modified by Kevin Liu 20160612 bug853
		unsigned char szTitle[17+1];
		GetEngTime(szTitle);
		sprintf(szName, "%.16s", szTitle);
		
	}
	else
	{
		sprintf(szName, "%.16s", _T(glTranConfig[glProcInfo.stTranLog.ucTranType].szLabel));
	}
}
...
{
	int		iRet;
	ShowLogs(1, "In ReadMagCardInfo Step 1");
	//if( glEdcMsgPtr->MsgType == MAGCARD_MSG )//Commented out by Wisdom
	if(0)
	{
		ShowLogs(1, "In ReadMagCardInfo Step 2");
	...
	{
		magstp = 1;
		ShowLogs(1, "In ReadMagCardInfo Step 3");
		glProcInfo.stTranLog.uiEntryMode = MODE_SWIPE_INPUT;
		MagRead(glProcInfo.szTrack1, glProcInfo.szTrack2, glProcInfo.szTrack3);
	}
	ShowLogs(1, "In ReadMagCardInfo Step 4");
	iRet = GetPanFromTrack(glProcInfo.stTranLog.szPan, glProcInfo.stTranLog.szExpDate);
	ShowLogs(1, "In ReadMagCardInfo Step 5");
	if( iRet!=0 )
	{
		ShowLogs(1, "In ReadMagCardInfo Step 6");
		DispMagReadErr();
		return ERR_NO_DISP;
	}
	ShowLogs(1, "In ReadMagCardInfo Step 7");
	Beep();
	ShowLogs(1, "In ReadMagCardInfo Step 8");
	return 0;
}
...
int ValidCard(void)
{
	...
	if( glProcInfo.stTranLog.ucTranType==INSTALMENT )
	{
		if( glProcInfo.bIsFallBack || glEdcMsgPtr->MsgType == ICCARD_MSG )
		{
			...
			Gui_ClearScr();
			PubBeepErr();
			Gui_ShowMsgBox(GetCurrTitle(), gl_stTitleAttr, _T("Swipe Again"), gl_stCenterAttr, GUI_BUTTON_OK, 5, NULL);
...
int SelectAcqForCard(const uchar *psAcqMatchFlag, uchar *pucCardIndex)
{
	...
	for(ucAcqNum=ucCnt=0; ucCnt<glSysParam.ucAcqNum; ucCnt++)
	{
		...
		sprintf(stAcqMenuItem[ucAcqNum].szText, "%.10s", glSysParam.stAcqList[ucCnt].szName);
...
{
	int		iRet;
	ShowLogs(1, "In SwipeCardProc Step 1");
	ShowLogs(1, "In SwipeCardProc Step 1b");
	iRet = ReadMagCardInfo();
	ShowLogs(1, "In SwipeCardProc Step 2");
	if( iRet!=0 )
	{
		ShowLogs(1, "In SwipeCardProc Step 2a");
		return iRet;
	}
	ShowLogs(1, "In SwipeCardProc Step 3");
	...
	{
		ShowLogs(1, "In SwipeCardProc Step 3a");
		return 0;
	}
	...
	{
		ShowLogs(1, "In SwipeCardProc Step 5");
		...
		else
		{
			...
			Gui_ClearScr();
			PubBeepErr();
			Gui_ShowMsgBox(szName, gl_stTitleAttr, _T("Re-swipe"), gl_stCenterAttr, GUI_BUTTON_OK, 3, NULL);
			return ERR_NO_DISP;
		}
	}
	else if( IsChipCardSvcCode(glProcInfo.szTrack2) )
	{
		ShowLogs(1, "In SwipeCardProc Step 6");
			...
			glProcInfo.stTranLog.ucTranType!=INSTALMENT )
		{
			Gui_ClearScr();
			PubBeepErr();
			Gui_ShowMsgBox(GetCurrTitle(), gl_stTitleAttr, _T("Insert Card"), gl_stCenterAttr, GUI_BUTTON_OK, 3, NULL);
			return ERR_NEED_INSERT;
		}
	}
	ShowLogs(1, "In SwipeCardProc Step 7");
	return 0;
}
...
int InsertCardProc(void)
{
	...
	ShowLogs(1, "InsertCardProc Step 1.");
	// 如果已经FALLBACK,忽略IC卡插卡操作
	// if did fallback, will ignore the coming inserting processing
	if( glProcInfo.bIsFallBack==TRUE )
	{
		glProcInfo.bIsFallBack==FALSE;//Wisdom
		ShowLogs(1, "InsertCardProc Step 1a.");
	...
	DispProcess();
	ShowLogs(1, "InsertCardProc Step 2.");
#ifdef ENABLE_EMV
	InitTransEMVCfg();
	ShowLogs(1, "InsertCardProc Step 3.");
#endif

	// 应用选择
	// EMV application selection. This is EMV kernel API
	iRet = EMVAppSelect(ICC_USER, glSysCtrl.ulSTAN);
	ShowLogs(1, "InsertCardProc Step 4: %d", iRet);
		...
		iRet==ICC_CMD_ERR  || iRet==EMV_RSP_ERR )
	{
		...
		ShowLogs(1, "InsertCardProc Step 4a.");
		return ERR_NEED_FALLBACK;
	}
	if( iRet==EMV_TIME_OUT || iRet==EMV_USER_CANCEL )
	{
		ShowLogs(1, "InsertCardProc Step 4b.");
	...
	iRet = ValidCard();
	ShowLogs(1, "InsertCardProc Step 8: %d", iRet);
	if( iRet!=0 )
	{
		return iRet;
	}*/
	...
	{
		ShowLogs(1, "InsertCardProc Step 11.");
		amtCount = 0;
		iRet = GetAmount();
		if( iRet!=0 )
		{
		...
		EMVSetTLVData(0x81, sTemp, 4);//by wisdom
	}
	ShowLogs(1, "InsertCardProc Step 12.");
	DispProcess();
	...
	{
		ShowLogs(1, "InsertCardProc Step 13.");
		iRet = EMVCardAuth();
		ShowLogs(1, "InsertCardProc Step 14: %d", iRet);
		if( iRet!=EMV_OK )
		{
			ShowLogs(1, "InsertCardProc Step 13a.");
			return ERR_TRAN_FAIL;
		}
	}
	ShowLogs(1, "InsertCardProc Step 14.");
	return 0;
}
#endif
...
int GetEmvTrackData(void)
{
	...
	//DispProcess();
	ShowLogs(1, "Getting EmvTrack Data");
	iRet = EMVReadAppData();
	if( iRet==EMV_TIME_OUT || iRet==EMV_USER_CANCEL )
	{
		return ERR_USERCANCEL;
	...
	ShowLogs(1, "Getting EmvTrack Data 2");
	...
	ShowLogs(1, "Getting EmvTrack Data 3");
	// read PAN
	bReadPan = FALSE;
	memset(sTemp, 0, sizeof(sTemp));
...
//			return ERR_TRAN_FAIL;
	...
	ShowLogs(1, "Getting EmvTrack Data 4");
	...
	ShowLogs(1, "Getting EmvTrack Data 5");
	...
	ShowLogs(1, "Getting EmvTrack Data 6");
    // application label
	...
	ShowLogs(1, "Getting EmvTrack Data 7");
	...
	ShowLogs(1, "Getting EmvTrack Data 8");
...
//	sprintf(stBuff[ucLines].szLine, "%.16s", glProcInfo.stTranLog.szHolderName);
	...
	displayName(txnname); //By Wisdom	
	Gui_CreateInfoPage(_T(txnname), gl_stTitleAttr, stBuff, ucLines, &stHexMsgPage);
	// Display message
...
int GetCard(uchar ucMode)
{
	...
	while( 1 )
	{
		...
		{
			ShowLogs(1, "Inside GetCard Key In");
			ucKey = GetLastKey();
			if( ucKey==KEYCANCEL )
			{
				Gui_ClearScr();//Wisdom
		...
		{
			//Start from fallback here
			ShowLogs(1, "Inside GetCard Card Swiped 1: %d", bCheckICC);
			ShowLogs(1, "Inside GetCard Card Swiped 1: %c", bCheckICC);
			iRet = SwipeCardProc(bCheckICC);
			ShowLogs(1, "Inside GetCard Card Swiped 2");
			if( iRet==0 )
			{
				ShowLogs(1, "Inside GetCard Card Swiped 3");
				return ValidCard();
			}
			else if( iRet==ERR_SWIPECARD )
			{
				ShowLogs(1, "Inside GetCard Card Swiped 4");
				DispMagReadErr();
			}
			else if( iRet==ERR_NEED_INSERT )
			{
				// 是芯片卡
				// IC card
				ShowLogs(1, "Inside GetCard Card Swiped 5");
			...
			{
				ShowLogs(1, "Inside GetCard Card Swiped 6");
				return iRet;
			}
		}
#ifdef ENABLE_EMV
		...
		{
			ShowLogs(1, "Inside GetCard Card Inserted");
			iRet = InsertCardProc();
			ShowLogs(1, "GetCard Step 4a: %d", iRet);
			if( iRet==0 )
			{
				ShowLogs(1, "GetCard Step 4b: %d", iRet);
				return 0;
			}
			else if( iRet==ERR_NEED_FALLBACK )
			{
				ShowLogs(1, "About Starting Fallback");
				DispFallBackPrompt();
				ShowLogs(1, "Done Displaying Fallback");
				PromptRemoveICC();
				ShowLogs(1, "Done Telling To Remove Icc 1");
				ucMode = CARD_SWIPED|FALLBACK_SWIPE;	// Now we don't support fallback to manual-PAN-entry
				ShowLogs(1, "Done Telling To Remove Icc 2");
			}
...
int DetectCardEvent(uchar ucMode)
{
	...
	if( ucMode & FALLBACK_SWIPE )
	{
		ShowLogs(1, "Inside DetectCardEvent Option 1");
		sprintf(szPrompt, "%s\n%s", _T("Swipe Card"), _T("Fall back"));
	}
	else if( (ucMode & CARD_SWIPED) && (ucMode & CARD_INSERTED) )
	{
		ShowLogs(1, "Inside DetectCardEvent Option 2");
...
//		DispIndicator();
	...
	{
		ShowLogs(1, "Inside DetectCardEvent Option 3");
	...
	{
		ShowLogs(1, "Inside DetectCardEvent Option 4");
		sprintf(szPrompt, "%s", _T("Swipe Card"));
	}
	
	sg_ucMode = ucMode;
	ShowLogs(1, "Inside DetectCardEvent Calling Callback 1");
	Gui_RegCallback(GUI_CALLBACK_LISTEN_EVENT, inputEvent);
	ShowLogs(1, "Inside DetectCardEvent Showing message");
	Gui_ShowMsgBox(GetCurrTitle(), gl_stTitleAttr, szPrompt, gl_stCenterAttr, GUI_BUTTON_CANCEL, -1, &key);
	ShowLogs(1, "Inside DetectCardEvent Calling Callback 2");
	Gui_RegCallback(GUI_CALLBACK_LISTEN_EVENT, NULL);
	if(key != NOKEY && key != 0){
	    sg_ucMode = CARD_KEYIN;
	    SetLastKey(key);
	}
	ShowLogs(1, "Inside DetectCardEvent Returning: %d", sg_ucMode);
	return sg_ucMode;
}
...
void DispFallBackPrompt(void)
{
	...
	sprintf(szBuff, "%s\n   %.*s", _T("Swipe Card"), strlen(_T("Pull Out")), "         ");
	sprintf(szBuff2, "%s\n   %s", _T("Swipe Card"), _T("Pull Out"));
...
int GetAmount(void)
{
	...
	char confee[13] = {0}, szBuff[128] = {0}, disp[128] = {0};
	uchar bSSL = 0;
    uchar szSSL[120];
    ...
    if(txnType == 14)
    {
    	...
    	ShowLogs(1, "paymentconveniencefee: %s", confee);
    	getConvenienceFee(glProcInfo.stTranLog.szTipAmount, confee);
    	ShowLogs(1, "Convenience Fee: %s", glProcInfo.stTranLog.szTipAmount);*/
    }
	...
	if( glProcInfo.stTranLog.szAmount[0]!=0 && amtCount == 1)
	{
		//Commented out by Wisdom
		
		ShowLogs(1, "AMOUNT CAUGHT");
		return 0;
		
	/*iRet = InputAmount(AMOUNT);
		if( iRet!=0 )
		{
			ShowLogs(1, "AMOUNT CAUGHT 2");
			return iRet;
		}*/
	}
	...
	while( 1 )
	{
		...
		//Use for cash back or tip
		if(txnType == 8)
    	{
			iRet = GetTipAmount();
			if( iRet!=0 )
			{
				ShowLogs(1, "AMOUNT CAUGHT 3");
				return iRet;
			}
		}

		ShowLogs(1, "1. Amount: %s", glProcInfo.stTranLog.szAmount);
		ShowLogs(1, "1. Cashback: %s", glProcInfo.stTranLog.szTipAmount);
		memcpy(glProcInfo.stTranLog.szInitialAmount, glProcInfo.stTranLog.szAmount, sizeof(glProcInfo.stTranLog.szInitialAmount));
		PubAscAdd(glProcInfo.stTranLog.szAmount, glProcInfo.stTranLog.szTipAmount, 12, szTotalAmt);
		/*if(txnType == 14)
	    {
	    	//Confirm because it is payment
	    	ShowLogs(1, "Payment Total: %s", szTotalAmt);
		...
		ShowLogs(1, "2. Cashback: %s", glProcInfo.stTranLog.szTipAmount);
...
TAG_SELECT_PLAN:
	...
	{
		sprintf((char *)stPlanMenuItems[ii].szText, "%s", (char *)glSysParam.stPlanList[ii].szName);
...
void App_ConvAmountTran(const uchar *pszIn, uchar *pszOut, uchar ucMisc)
{
	...
	strcpy((char *)szBuff, (char *)store_env);
...
int DisplayInputAmount(gui_callbacktype_t type, void *data, int *dataLen)
{
    if(GUI_CALLBACK_UPDATE_TEXT == type && data && dataLen){
        ...
        if(0 == amount->pStr[0] || IsNumStr(amount->pStr))
        {
			...
			//removed by KevinLiu 20160720 bug MEDC-12
...
int InputAmount(uchar ucAmtType)
{
    ...
    //Used to check payarena
    if(txnType == 15 || txnType == 14)
    {
    	ShowLogs(1, "InputAmount: I am the one Stoping you");
    	return 0;
    }

	ShowLogs(1, "InputAmount: Trying to get Amount Step 1");
	amtCount = 1;
	...
	case TIPAMOUNT:
		strcpy(szBuff, _T("CASHBACK"));
		break;
	}
	strcat(szBuff, _T(" ("));
	memset(store_env, '\0', strlen(store_env));
    UtilGetEnv("curabbr", store_env);
	//strncat(szBuff,glSysParam.stEdcInfo.stLocalCurrency.szName, 3);
	...
	ShowLogs(1, "InputAmount: Trying to get Amount Step 2");
	//For Ecr
	if(1)
	{
		while(1)
		{
			...
			ShowLogs(1, "InputAmount: Trying to get Amount Step 3");
...
int GetPIN(uchar ucFlag)
{
	...
	if( !(ucFlag & GETPIN_EMV) )
	{
		if ( (glProcInfo.stTranLog.uiEntryMode & MODE_CHIP_INPUT) )
		{
			ShowLogs(1, "GetPin Step 1b");
			return 0;
		}
		else if( !ChkIssuerOption(ISSUER_EN_PIN) )
		{
			ShowLogs(1, "GetPin Step 1c");
			return 0;
		}
	}
	ShowLogs(1, "GetPin Step 2");
	...
	ShowLogs(1, "GetPin Step 3");
	if( iRet!=0 )
	{
		ShowLogs(1, "GetPin Step 3a");
		return iRet;
	}
	ShowLogs(1, "GetPin Step 4");
	PubAscAdd(glProcInfo.stTranLog.szAmount, glProcInfo.stTranLog.szTipAmount, 12, szTotalAmt);
	ShowLogs(1, "GetPin Step 5");
	ucFlag &= (uchar)(0xFF-GETPIN_EMV);
	ShowLogs(1, "GetPin Step 6");
	GetDispAmount(szTotalAmt, szDispAmt);
	ShowLogs(1, "GetPin Step 7");
	strcat(szDispAmt, "\n");
	ShowLogs(1, "GetPin Step 8");
	if( ucFlag==0 )
	{
		strcat(szDispAmt, _T("Enter Pin"));
	}
	else
	{
		strcat(szDispAmt, _T("Wrong Pin, Retry"));
	}
	//added by Kevin Liu 20160614 bug851 
	strcat(szDispAmt, "\n\n\n\n\n\n\n\n");
	ShowLogs(1, "GetPin Step 9");
	Gui_ClearScr();
	Gui_ShowMsgBox(GetCurrTitle(), gl_stTitleAttr, szDispAmt, st_Small_Left, GUI_BUTTON_NONE, 0, NULL);
	ShowLogs(1, "GetPin Step 10");
	// show prompt message on PINPAD
	if (ChkTermPEDMode(PED_EXT_PP))
	{
		ShowLogs(1, "GetPin Step 11");
		ucAmtFlag = 0;
		if( glProcInfo.stTranLog.ucTranType==VOID || glProcInfo.stTranLog.ucTranType==REFUND )
		{
		...
		ShowLogs(1, "GetPin Step 12");
		// show amount on PINPAD
		Gui_ClearScr();
		Gui_ShowMsgBox(GetCurrTitle(), gl_stTitleAttr, "(Use Pinpad)", gl_stCenterAttr, GUI_BUTTON_NONE, 0, NULL);
		PPScrCls();
		PPScrPrint(0, 0, szBuff);
		ShowLogs(1, "GetPin Step 13");
	}

	if (ChkTermPEDMode(PED_INT_PCI))
	{
		ShowLogs(1, "GetPin Step 14");
		ScrGotoxy(32, 6); // TODO: it should base on screen size Kim_LinHB
	}

	memset(glProcInfo.sPinBlock, 0, sizeof(glProcInfo.sPinBlock));
	ShowLogs(1, "GetPin Step 15");
	...
	{
		char lasthost[128] = {0};
		ShowLogs(1, "GetPin Step 16");
		//iRet = PedGetPinBlock(ucPinKeyID, "0,4,5,6,7,8", szPAN, glProcInfo.sPinBlock, 0, USER_OPER_TIMEOUT*1000);
		//This is working
		//iRet = PedGetPinBlock(2, "4,6", szPAN, glProcInfo.sPinBlock, 0, 30000);//By Wisdom
		...
		{	
			if((glProcInfo.stTranLog.szPan[0] == '5') && (glProcInfo.stTranLog.szPan[1] == '0') && (glProcInfo.stTranLog.szPan[2] == '6'))
				iRet = PedGetPinBlock(2, "4,6", szPAN, glProcInfo.sPinBlock, 0, 30000);//By Wisdom
			else
				iRet = PedGetPinBlock(3, "4,6", szPAN, glProcInfo.sPinBlock, 0, 30000);//By Wisdom
		}else
		{
			//What was working
			iRet = PedGetPinBlock(2, "4,6", szPAN, glProcInfo.sPinBlock, 0, 30000);//By Wisdom
		}
		ShowLogs(1, "Pin Response: %d", iRet);

		HexEnCodeMethod(glProcInfo.sPinBlock, 8, pinBlockBuffer);
		strcpy(glSendPack.szPinBlock, pinBlockBuffer);
		ShowLogs(1, "GetPin Step 17: pan %s %d == %s", szPAN, iRet, pinBlockBuffer);
		if( iRet==0 )
		{
			pincheck = 1;
			glProcInfo.stTranLog.uiEntryMode |= MODE_PIN_INPUT;
			ShowLogs(1, "GetPin Step 17a");
			return 0;
		}
		else if (iRet==PED_RET_ERR_NO_PIN_INPUT)	// !!!! PIN bypass
		{
			ShowLogs(1, "GetPin Step 18");
			return 0;
		}
		else if( iRet==PED_RET_ERR_INPUT_CANCEL )	// !!!! cancel input
		{
			ShowLogs(1, "GetPin Step 19");
			return ERR_USERCANCEL;
		}

		DispPciErrMsg(iRet);
		ShowLogs(1, "GetPin Step 20");
		return ERR_NO_DISP;
	}
	ShowLogs(1, "GetPin Step 21");
	//-------------- External PCI PED --------------
...
void DispPciErrMsg(int iErrCode)
{
	...
	case PED_RET_ERR_NO_KEY:
		sprintf(szDispBuff, _T("No Keys"));
		break;

	case PED_RET_ERR_KEYIDX_ERR:
		sprintf(szDispBuff, _T("Wrong Key Index"));
		break;

	case PED_RET_ERR_DERIVE_ERR:
		sprintf(szDispBuff, _T("Wrong Key Level"));
	    break;
...
void MaskPan(const uchar *pszInPan, uchar *pszOutPan)
{
	...
	if(0)//By Wisdom for UP
	{	
		ShowLogs(1, "Masking Pan the right align");
		// right align
	...
	{	
		ShowLogs(1, "Masking Pan the left align");
		// left align
		for(iCnt=0; iCnt<iPanLen; iCnt++)
		{
...
int SelectAcq(uchar bAllowSelAll, const uchar *pszTitle, uchar *pucAcqIndex)
{
	...
	for(ucCnt=0; ucCnt<glSysParam.ucAcqNum; ucCnt++)
	{
		sprintf((char *)stAcqMenuItem[iMenuNum].szText, "%3.3s %.10s",
...
int SelectIssuer(uchar *pucIssuerIndex)
{
	...
	{
		sprintf((char *)stIssuerMenuItem[ucCnt].szText, "%.10s", glSysParam.stIssuerList[ucCnt].szName);
...
const char *GetCurSignPrefix(uchar ucAcqKey)
{
    ...
    if(ACQ_ALL == ucAcqKey)
        ...
        sprintf(szPath, "SIG-%d-%d", ucAcqKey, glCurAcq.ulCurBatchNo);
    return szPath;
}
char HasE_Signature()
{
...
void DoE_Signature()
{
	ShowLogs(1, "Signature is here");
	return;
    if((glSysParam.sTermInfo[19] & 0x01) == 1)
    { // touch screen, focus to get the E signature
		int iRet;
		sprintf(glProcInfo.stTranLog.szSignPath, "%s-%d", GetCurSignPrefix(glCurAcq.ucKey), glProcInfo.stTranLog.ulInvoiceNo);
        Gui_ClearScr();
