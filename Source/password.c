...
uchar PasswordSub(uchar ucPwdID)
{
	...
	ShowLogs(1, "PasswordSub Step 1");
	if( ucPwdID>sizeof(sgPwdInfo)/sizeof(sgPwdInfo[0]) )
	{
		ShowLogs(1, "PasswordSub Step 2");
		return 1;
	}

	while( 1 )
	{
		ShowLogs(1, "PasswordSub Step 3");
		...
		if(GUI_OK != Gui_ShowInputBox(GetCurrTitle(), gl_stTitleAttr, 
			...
			Gui_ClearScr();
			ShowLogs(1, "PasswordSub Step 4");
			return 1;
		}

		ShowLogs(1, "PasswordSub Step 3. Value: %s", szBuff);
		...
		if( memcmp(szBuff, "0000", 4)==0 &&
			strlen((char *)szBuff)==4 )
		{
			ShowLogs(1, "PasswordSub Step 5");
			return 0;
		}
