...
void SetEntryModePre(const TRAN_LOG *pstLog)
{
	...
	if( ChkIfAmex() )
	{
		...
		SetAmexEntryModePre(pstLog);
	}
	else
	{
		//ShowLogs(1, "Inside SetEntryMode 2");
		SetStdEntryModePre(pstLog);
	}
}

void SetAmexEntryModePre(const TRAN_LOG *pstLog)
{
...
void SetStdEntryModePre(const TRAN_LOG *pstLog)
{
	//------------------------------------------------------------------------------
	// Entry mode digit 1
	#ifdef ENABLE_EMV
...
void SetCommReqFieldPre(void)
{
	...
	//Start here	
	SetEntryModePre(&glProcInfo.stTranLog);		// bit 22, entry mode
	SetCondCodePre();		// bit 25, service condition code 
...
int preAuth()
{
	...
	while(1)
	{
		...
		{
			txnType = 3;
			UtilPutEnv("actype", "60");//PreAuth
			iRet = PreauthProcICCMsg();
