...
{
	// Wireless error
	{WL_RET_ERR_PORTINUSE,	_T_NOOP("INTERNAL ERR")},	// ģ��ڱ�ռ��
	{WL_RET_ERR_NORSP,		_T_NOOP("INTERNAL ERR")},	// ģ��û��Ӧ��
	{WL_RET_ERR_RSPERR,		_T_NOOP("INTERNAL ERR")},	// ģ��Ӧ�����
	{WL_RET_ERR_PORTNOOPEN,	_T_NOOP("INTERNAL ERR")},	// ģ�鴮��û�д�
	{WL_RET_ERR_NEEDPIN,	_T_NOOP("NEED SIM PIN")},	// ��ҪPIN��
	{WL_RET_ERR_NEEDPUK,	_T_NOOP("NEED SIM PUK")},	// ��ҪPUK��PIN��
	{WL_RET_ERR_PARAMER,	_T_NOOP("INVALID CONFIG")},	// ��������
	{WL_RET_ERR_ERRPIN,		_T_NOOP("INVALID SIMPIN")},	// �������
	{WL_RET_ERR_NOSIM,		_T_NOOP("NO SIM CARD")},	// û�в���SIM��
	{WL_RET_ERR_NOTYPE,		_T_NOOP("INVALID TYPE")},	// ��֧�ֵ�����
	{WL_RET_ERR_NOREG,		_T_NOOP("SIM NOT REG.")},	// ����û��ע��
//	{WL_RET_ERR_INIT_ONCE,	_T_NOOP("INITED.")},		// ģ���ѳ�ʼ��
	{WL_RET_ERR_LINEOFF,	_T_NOOP("LINE OFF")},		// ���ӶϿ�
	{WL_RET_ERR_TIMEOUT,	_T_NOOP("TIMEOUT")},		// ��ʱ
	{WL_RET_ERR_REGING,		_T_NOOP("REGING")},			// ����ע����
	{WL_RET_ERR_PORTCLOSE,	_T_NOOP("INTERNAL ERR")},	// �رմ��ڳ���
	{WL_RET_ERR_MODEVER,	_T_NOOP("INTERNAL ERR")},	// �����ģ��汾
//	{WL_RET_ERR_DIALING,	_T_NOOP("INTERNAL ERR")},	// ������  
	{WL_RET_ERR_ONHOOK,		_T_NOOP("INTERNAL ERR")},	// �ػ���
	{WL_RET_ERR_PPP_BRK,	_T_NOOP("DISCONNECTED")},	// ����PPP�Ͽ�
	{WL_RET_ERR_NOSIG,		_T_NOOP("NO SIGNAL")},		// �������ź�
	{WL_RET_ERR_POWEROFF,	_T_NOOP("INTERNAL ERR")},	// ģ�����µ�
	{WL_RET_ERR_BUSY,		_T_NOOP("INTERNAL ERR")},	// ģ��æ
	{WL_RET_ERR_OTHER,		_T_NOOP("INTERNAL ERR")},	// ����δ֪����
	// TCPIP stack error
	{NET_ERR_MEM,			_T_NOOP("INTERNAL ERR")},	// �ڴ治��
	{NET_ERR_BUF,			_T_NOOP("INTERNAL ERR")},	// ����������
	{NET_ERR_ABRT,			_T_NOOP("CONN. FAIL")},		// ��ͼ��������ʧ��
	{NET_ERR_RST,			_T_NOOP("CONN. CLOSED")},	// ���ӱ��Է���λ���յ��Է���Reset��
	{NET_ERR_CLSD,			_T_NOOP("CONN. CLOSED")},	// �����ѹر�
	{NET_ERR_CONN,			_T_NOOP("CONN. FAIL")},		// ����δ�ɹ�����
	{NET_ERR_VAL,			_T_NOOP("INVALID PARAM")},	// �������
	{NET_ERR_ARG,			_T_NOOP("INVALID PARAM")},	// �������
	{NET_ERR_RTE,			_T_NOOP("INTERNAL ERR")},	// ����·��(route)
	{NET_ERR_USE,			_T_NOOP("INTERNAL ERR")},	// ��ַ���˿�ʹ����
	{NET_ERR_IF,			_T_NOOP("INTERNAL ERR")},	// �ײ�Ӳ������
//	{NET_ERR_ISCONN										// �����ѽ���
	{NET_ERR_TIMEOUT,		_T_NOOP("TIMEOUT")},		// ��ʱ
	{NET_ERR_AGAIN,			_T_NOOP("PLS RETRY")},		// ������Դ�����ڣ�������
//	{NET_ERR_EXIST										// �Ѵ���
	{NET_ERR_SYS,			_T_NOOP("UNSUPPORTED")},	// ϵͳ��֧��
	{NET_ERR_PASSWD,		_T_NOOP("INVALID PWD")},	// ��������
	{NET_ERR_MODEM,			_T_NOOP("DIAL FAIL")},		// ����ʧ��
	{NET_ERR_LINKDOWN,		_T_NOOP("LINK DOWN")},		// ������·�ѶϿ��������²���
	{NET_ERR_LOGOUT,		_T_NOOP("LOG OUT")},		// Logout
	{NET_ERR_PPP,			_T_NOOP("CONN. FAIL")},		// PPP�Ͽ�
...
int CommInitWifi(COMM_CONFIG *pstCfg)
{
	...
	CommSetCfgParam(pstCfg);
	iRet = WifiCheck(NULL);
	ShowLogs(1, "1. WIFICHECK RETURNED: %d", iRet);
	if(iRet > 0)
		return 0;
	else if (-3 == iRet)
	{
	...
	Gui_ClearScr();
	Gui_ShowMsgBox("", gl_stTitleAttr, _T("Please Wait"), gl_stCenterAttr, GUI_BUTTON_NONE, 0, NULL);
	...
	TimerSet(TIMER_TEMPORARY, 600); // 20 sec for checking link //200
	while(1)
	{
		iRet = WifiCheck(NULL);
		//ShowLogs(1, "3. WIFICHECK RETURNED: %d", iRet);
		...
		if (iRet < 0)
		{
			ShowLogs(1, "2. WIFICHECK RETURNED: %d", iRet);
			return iRet;
		}
		else
		{
			...
			Gui_ClearScr();
			Gui_ShowMsgBox("", gl_stTitleAttr, szPrompt, gl_stCenterAttr, GUI_BUTTON_OK, 1, NULL);
...
int CommInitModule(COMM_CONFIG *pstCfg)
{
	...
	ShowLogs(1, "Initializing Comms. Type: %d", pstCfg->ucCommType);
	...
	case CT_WCDMA:   // added by Gillian 2015/11/23
		for (ii=0; ii<3; ii++)
		{
			ShowLogs(1, "Trying to initialize CommInitGprsCdma");
			iRet = CommInitGprsCdma(pstCfg);
			ShowLogs(1, "Trying to initialize CommInitGprsCdma. Return: %d", iRet);
			if (iRet==0)
			{
				break;
			}
...
int CommSetCfgParam(const COMM_CONFIG *pstCfg)
{
	...
	case CT_WIFI:
		ShowLogs(1, "Inside CommSetCfgParam 1");
		if( !ChkIfValidIp(pstCfg->stWifiPara.stHost1.szIP) ||
			!ChkIfValidPort(pstCfg->stWifiPara.stHost1.szPort))
		{
			ShowLogs(1, "Inside CommSetCfgParam Err");
			return ERR_COMM_INV_PARAM;
		}
		ShowLogs(1, "Inside CommSetCfgParam 2");
		break;
...
int reprocessWifi(uchar ucDialMode)
{
	int count = 0;
	int iRet = -1;
	int s = 0, i = 0;
	ShowLogs(1, "Inside reprocessWifi");
	while(count < 2)
	{
		RouteSetDefault(12); //MEDC-10
		ShowLogs(1, "Inside WIfi Dail 2");
		iRet = WIFIDial(ucDialMode);
		if (iRet == 0)
		{
			ShowLogs(1, "Inside WIfi Dail 3");
			return iRet;
		}
		ShowLogs(1, "Inside WIfi Dail 4");
		while(s < 2)
		{
			useExisting = 1;
			cancelWifi = 0;
			if(wifiSetup())
			{
				i = CommInitModule(&glSysParam.stTxnCommCfg);
		 		ShowLogs(1, "CommsInitialization Response: %d", i);
				if (iRet != 0)
				{
				...
				ShowLogs(1, "Done Calling CommSetCfgParam");
		 		i = CommDial(DM_DIAL);
		 		ShowLogs(1, "Done Calling CommDial: %d", i);
				return i;
			}
			if(cancelWifi == 1 || s >= 2)
			{
				...
				memcpy(glSysParam.stTxnCommCfg.stWirlessPara.szAPN, "web.gprs.mtnnigeria.net", strlen("web.gprs.mtnnigeria.net"));
				memcpy(glSysParam.stTxnCommCfg.stWirlessPara.szUID, "web", strlen("web"));
				memcpy(glSysParam.stTxnCommCfg.stWirlessPara.szPwd, "web", strlen("web"));
				
				memset(glSysParam.stTxnCommCfg.stWirlessPara.stHost1.szIP, '\0', sizeof(glSysParam.stTxnCommCfg.stWirlessPara.stHost1.szIP));
				memcpy(glSysParam.stTxnCommCfg.stWirlessPara.stHost1.szIP, glSysParam.stTxnCommCfg.stWifiPara.stHost1.szIP, strlen(glSysParam.stTxnCommCfg.stWifiPara.stHost1.szIP));
				memset(glSysParam.stTxnCommCfg.stWirlessPara.stHost2.szIP, '\0', sizeof(glSysParam.stTxnCommCfg.stWirlessPara.stHost2.szIP));
				memcpy(glSysParam.stTxnCommCfg.stWirlessPara.stHost2.szIP, glSysParam.stTxnCommCfg.stWifiPara.stHost1.szIP, strlen(glSysParam.stTxnCommCfg.stWifiPara.stHost1.szIP));
				...
				memcpy(glSysParam.stTxnCommCfg.stWirlessPara.stHost1.szPort, glSysParam.stTxnCommCfg.stWifiPara.stHost1.szPort, strlen(glSysParam.stTxnCommCfg.stWifiPara.stHost1.szPort));
				...
				memcpy(glSysParam.stTxnCommCfg.stWirlessPara.stHost2.szPort, glSysParam.stTxnCommCfg.stWifiPara.stHost1.szPort, strlen(glSysParam.stTxnCommCfg.stWifiPara.stHost1.szPort));

				glSysParam.stTxnCommCfg.ucCommType = 5;
				DisplayInfoNone("", "Reverting", 1);
...
int CommDial(uchar ucDialMode)
{
	...
	case CT_WIFI:
		...
		//ShowLogs(1, "Inside WIfi Dail 1");
		//iRet = reprocessWifi(ucDialMode);
	...
	case CT_CDMA:
	case CT_GPRS:
		ShowLogs(1, "Inside Comm Dial XXXXXX");
		RouteSetDefault(11);
		iRet = SXXWlDial(&sg_stCurCfg.stWirlessPara, 60, 3600, ucDialMode);
		ShowLogs(1, "Inside Comm Dial. Return: %d", iRet);
		break;
...
int CommTxd(const uchar *psTxdData, ushort uiDataLen, ushort uiTimeOutSec)
{
	...
	case CT_WIFI:
		ShowLogs(1, "About Sending Via Wifi");
...
int CommOnHookGPRS(uchar bReleaseAll)
{
	int		iRet;
...
int CommOnHook(uchar bReleaseAll)
{
	...
	ShowLogs(1, "CommOnHook Dont close the sockets");
	return 1;
...
int CommOnHookCustom(uchar bReleaseAll)
{
	int		iRet;
	
	ShowLogs(1, "CommOnHookCustom Dont close anything");
	return 1;
	...
	case CT_MODEM:
		...
		break;
   	case CT_WIFI:
   		ShowLogs(1, "CLOSING FOR WIFI");
		iRet = WIFIOnHook(bSSL, bReleaseAll);
		break;
	case CT_TCPIP:
		ShowLogs(1, "CLOSING FOR TCP");
		iRet = TcpOnHook(bSSL, bReleaseAll);
		break;
	...
	//SxxSSLCloseCustom();//Wisdom for SSL
