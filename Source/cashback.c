...
void SetEntryModeCB(const TRAN_LOG *pstLog)
{
	...
	if( ChkIfAmex() )
	{
		...
		SetAmexEntryModeCB(pstLog);
	}
	else
	{
		//ShowLogs(1, "Inside SetEntryMode 2");
		SetStdEntryModeCB(pstLog);
	}
}

void SetAmexEntryModeCB(const TRAN_LOG *pstLog)
{
...
void SetStdEntryModeCB(const TRAN_LOG *pstLog)
{
	//------------------------------------------------------------------------------
	// Entry mode digit 1
	#ifdef ENABLE_EMV
...
void SetCommReqFieldCB(void)
{
	...
	//sprintf((char *)glSendPack.szTransFee, "%.*s", LEN_TRANS_FEE, "C00010000");
	...
	if( !ChkIfZeroAmt(glProcInfo.stTranLog.szTipAmount) )
	{
		...
		sprintf((char *)glSendPack.szTransFee, "C%s", glProcInfo.stTranLog.szTipAmount + 4);
	...
	else
	{
		sprintf((char *)glSendPack.szTranAmt, "%.*s", LEN_TRAN_AMT, glProcInfo.stTranLog.szAmount);
		ShowLogs(1, "Amount: %s", glSendPack.szTranAmt);
		sprintf((char *)glSendPack.szTransFee, "C%s", "00000000");
		sprintf((char *)glSendPack.szExtAmount, "%s", "000000000000");
		ShowLogs(1, "Transaction Amount: %s", glSendPack.szTransFee);
	...
	//Start here	
	SetEntryModeCB(&glProcInfo.stTranLog);		// bit 22, entry mode
	SetCondCodeCB();		// bit 25, service condition code 
...
int TransCaptureCB(void)
{
	int	iRet;
	char name[50] = {0};
	glProcInfo.stTranLog.ucTranType = CASHADVANCE;
	displayName(name);
...
int cashBack()
{
	...
	while(1)
	{
		...
		{
			txnType = 8;
			UtilPutEnv("actype", "09");//Cash Advance
			iRet = CashbackProcICCMsg();
