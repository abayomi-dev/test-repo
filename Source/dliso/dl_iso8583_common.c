								    ...
								    DL_UINT8       **oPtr )
{
	...
	*oPtr = NULL;
	...
	{
		//ShowLogs(1, "_DL_ISO8583_MSG_AllocField 2");
		/* we cannot over-write an existing entry in static mode */
		if ( NULL != ioMsg->field[iField].ptr ) /* error */
		{
			//ShowLogs(1, "_DL_ISO8583_MSG_AllocField Error 1");
			return kDL_ERR_OTHER;
		}
		else /* ok */
		{
			//ShowLogs(1, "_DL_ISO8583_MSG_AllocField 5");
			/* allocate from static buffer */
			if ( (ioMsg->sPtrNext + iSize + 1) > ioMsg->sPtrEnd ) /* error */
			{
				//ShowLogs(1, "_DL_ISO8583_MSG_AllocField Error 2");
				return kDL_ERR_OTHER;
			}
			else /* ok */
			{
				//ShowLogs(1, "_DL_ISO8583_MSG_AllocField 6");
	...
	{
		//ShowLogs(1, "_DL_ISO8583_MSG_AllocField DL_MEM_malloc 1");
		err = DL_MEM_malloc(iSize+1,&tmpPtr);
		//ShowLogs(1, "_DL_ISO8583_MSG_AllocField DL_MEM_malloc: %d", err);
		if ( !err )
		{
			//ShowLogs(1, "_DL_ISO8583_MSG_AllocField After DL_MEM_malloc.");
			DL_MEM_free(ioMsg->field[iField].ptr);
			ioMsg->field[iField].len = iSize;
			ioMsg->field[iField].ptr = tmpPtr;
		}
	}
	//ShowLogs(1, "_DL_ISO8583_MSG_AllocField End");
	*oPtr = tmpPtr;
	//ShowLogs(1, "_DL_ISO8583_MSG_AllocField Normal");
	return err;
}
