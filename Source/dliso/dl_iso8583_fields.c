...
void hexStringToByteUtils(char *output, const char *input, int len, int *binLen) {
	int j = 0;
	int i;
	char tmp[3] = { 0x0 };
	*binLen = 0;
...
void HexDecodeMethod2(unsigned char *psIn, unsigned int uiLength, unsigned char *psOut, int outLen) {
	...
	for (i = 0; (i < uiLength) && (i / 2 < outLen); i += 2) {
		tmp = psIn[i];
		if (tmp > '9') {
			tmp = (unsigned char) toupper((int) tmp) - 'A' + 0x0A;
		} else {
						  ...
						  DL_UINT8                   **ioPtr )
{
	...
	if ( !err )
	{
		if ( (actLen/2) > reqLen ) /* too long */
		{
			err = kDL_ERR_OTHER;
		}
		else if ( (actLen/2) == reqLen ) /* exact size */
		{
			tempData = (DL_UINT8* )sysalloc_calloc(reqLen,sizeof(DL_UINT8 ));
			//tempData = (DL_UINT8* )calloc(reqLen,sizeof(DL_UINT8 ));
			HexDecodeMethod(dataPtr,actLen,tempData);
			//hexStringToByteUtils(tempData,dataPtr,actLen);
			/* copy up to 'required' amount */
			DL_MEM_memcpy(tmpPtr,tempData,reqLen);
			sysalloc_free(tempData);
			//free(tempData);
			tmpPtr += reqLen;
		}
		else /* shorter - so need to right pad (space) */
		{
			tempData = (DL_UINT8* )sysalloc_calloc(reqLen,sizeof(DL_UINT8 ));
			//tempData = (DL_UINT8* )calloc(reqLen,sizeof(DL_UINT8 ));
			hexStringToByteUtils(tempData,dataPtr,actLen, &dataLen);
			/* copy what data we have (actual length) */
			DL_MEM_memcpy(tmpPtr,tempData,actLen);
			/* right pad as required */
			DL_MEM_memset(tmpPtr+actLen,(int)0,reqLen-(actLen/2));
			sysalloc_free(tempData);
			//free(tempData);
			tmpPtr += reqLen;
		}
