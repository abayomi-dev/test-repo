...
int PrnBmp(unsigned char *filename,int mode,char alignment, unsigned char *gMallocBuffer)
{
	...
	ShowLogs(1, "PrnBmp: %s. Filesize: %lu.", filename, filesize(filename));

	LineByte=filesize(filename);
	if(LineByte < 0)
	{
		ShowLogs(1, "PrnBmp Error 1");
		return -2;
	}

	fp=open(filename,O_RDWR);
	if(fp < 0) {
		ShowLogs(1, "PrnBmp Error 2");
		return -1;
	}
	...
	ShowLogs(1, "PrnBmp: iBmpPixWidth = %d", iBmpPixWidth);
	
	/*if(iBmpPixWidth>384) {
		free(bmp);
		ShowLogs(1, "PrnBmp Error 4");
		return  DATA_ERROR4;
	}*/

	iBmpPixBitCount = ((int)bmp[0x1d]) * 256 + ((int)bmp[0x1c]);
	ShowLogs(1, "PrnBmp: iBmpPixBitCount = %d", iBmpPixBitCount);
	/*if(iBmpPixBitCount != 1) {
		free(bmp);
		ShowLogs(1, "PrnBmp Error 5");
		return DATA_ERROR2;
	}*/
	...
	iBmpByteSize = ((int)bmp[0x25]) * 16777216 +
		...
		((int)bmp [0x23]) * 256  +
		(int)bmp [0x22];
	ShowLogs(1, "PrnBmp: iBmpByteSize = %d", iBmpByteSize);

	/*if(iBmpByteSize > 36864) {
		free(bmp);
		ShowLogs(1, "PrnBmp Error 6");
		return DATA_ERROR3;
	}*/
