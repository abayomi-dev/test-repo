...
int PubFReadN(int fd, void *psData, int iNeedBytes)
{
	...
	while(iLeftBytes>0)
	{
		...
		if (iReadBytes<=0)
		{
			ShowLogs(1, "Read Error: %d", iReadBytes);
			break;
		}
		iLeftBytes -= iReadBytes;
		psTmp	   += iReadBytes;
	}
	ShowLogs(1, "Length of psData: %d", strlen(psData));
...
uchar PubFileRead(const uchar *pszFileName, long lOffset, void *psData, int iLen)
{
	...
	if (fd<0)
	{
		ShowLogs(1, "Open: %s", pszFileName);
		ShowLogs(1, "Error: %d", fd);
		return 1;
	}
	...
	{
		close(fd);
		ShowLogs(1, "Seek: %s,%ld", pszFileName, lOffset);
		ShowLogs(1, "Error: %d", iRet);
		return 2;
	}
	
	iReadBytes = PubFReadN(fd, psData, iLen);
	ShowLogs(1, "Read Length: %d", iReadBytes);
	close(fd);
	if (iReadBytes!=iLen)
	{
		return 3;
...
int PubFWriteN(int fd, const void *psData, int iNeedBytes)
{
	...
	while(iLeftBytes>0)
	{
		...
		if (iWriteBytes<=0)
		{
			ShowLogs(1, "FWN: %d,%d", iWriteBytes, iLeftBytes);
			break;
		}
	...
	ShowLogs(1, "Writen Length: %d", iNeedBytes-iLeftBytes);
...
uchar PubFileWrite(const uchar *pszFileName, long lOffset, const void *psData, int iLen)
{
	...
	if (iWriteBytes!=iLen)
	{
		ShowLogs(1, "1. Written: %d,%d", iWriteBytes, iLen);
		return 3;
	}
	
	ShowLogs(1, "Written: %d to file %s", iWriteBytes, pszFileName);
