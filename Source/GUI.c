...
static int sg_nScrWidth = 0;
//static unsigned char sg_isColorScreen;
unsigned char sg_isColorScreen = 0;
int sg_nScrHeight = 0;
...
#endif
static ST_FONT sgFontS[3] = { { CHARSET_WEST, 6, 8, 0, 0 }, { CHARSET_WEST, 8, 16, 0, 0 }, { CHARSET_WEST, 12, 24, 0, 0 }, { CHARSET_GB2312, 20, 48, 0, 0 },};
//static ST_FONT sgFontS[3] = { { CHARSET_WEST, 6, 8, 0, 0 }, { CHARSET_WEST, 8, 16, 0, 0 }, { CHARSET_WEST, 12, 24, 0, 0 }, };
static ST_FONT sgFontM[3] = { { CHARSET_WEST, 6, 8, 0, 0 }, { CHARSET_WEST, 8, 16, 0, 0 }, { CHARSET_WEST, 12, 24, 0, 0 }, { CHARSET_GB2312, 20, 48, 0, 0 },};
...
int Gui_Init(unsigned int nBgColor, unsigned int nColor, const unsigned char *pszArabicFile) {
    if (sg_bStartup) {
        ...
        ShowLogs(1, "Wisdom is making evering Black and White");
        //sg_isColorScreen = 0;//By Wisdom
        ...
        if(menuFlag)
        {
            ////////ShowLogs(1, "Trying to clear flags");
        ...
        {
            ////////ShowLogs(1, "Trying to set flags");
            SetKeybyIndex(KEYCANCEL, "CANCEL");//CANCEL
            SetKeybyIndex(KEYCLEAR, "CLEAR");
            SetKeybyIndex(KEYENTER, "ENTER");//ENTER
            SetKeybyIndex(KEYMENU, "MENU");
            SetKeybyIndex(KEYFN, "FUNC");
            SetKeybyIndex(KEYALPHA, "ALPHA");
            SetKeybyIndex(KEYUP, "UP");//UP
            SetKeybyIndex(KEYDOWN, "DN");//DN
            SetKeybyIndex(GUI_KEYPREV, "1.PREV");
            SetKeybyIndex(GUI_KEYNEXT, "2.NEXT");
        }
    ...
    if (sg_isColorScreen) {
        sg_nDefColor = nColor;
        ////////ShowLogs(1, "sg_nDefColor: %d", sg_nDefColor);
        ////////ShowLogs(1, "sg_nDefBg: %d", sg_nDefBg);
...
int Gui_DrawText(const unsigned char *pszText, const GUI_TEXT_ATTR stTextAttr, unsigned int x_percent, unsigned int y_percent) {
    if (sg_bStartup) {
        ////////ShowLogs(1, "Failed inside startup check");
        return GUI_ERR_NOTINIT;
    }
    ...
    if (!pszText || !IsValid(stTextAttr.eAlign, GUI_ALIGN_LEFT, GUI_ALIGN_RIGHT) || !IsValid(stTextAttr.eFontSize, GUI_FONT_SMALL, GUI_FONT_LARGE) || x_percent > 100 || y_percent > 100) {
        ////////ShowLogs(1, "Failed inside verify parameters");
        return GUI_ERR_INVALIDPARAM;
    }
...
int Gui_ShowMsgBox(const unsigned char *pszTitle, GUI_TEXT_ATTR stTitleAttr, const unsigned char *pszContent, GUI_TEXT_ATTR stContentAttr, enum MSGBOXTYPE eType, int timeoutSec, int *pucKeyValue) {
    ...
    if(strlen(pszTitle) > 3)
    {
        strcpy(head, pszTitle);
    }else
    {
...
// verify parameter
    if (head && (!IsValid(stTitleAttr.eAlign, GUI_ALIGN_LEFT, GUI_ALIGN_RIGHT) || !IsValid(stTitleAttr.eFontSize, GUI_FONT_SMALL, GUI_FONT_LARGE))) {
        return GUI_ERR_INVALIDPARAM;
    }
    ...
    //title
    if (head) {
        DrawText((void *) head, (void *) &stTitleAttr, sgTitleArea.top, sgTitleArea.bottom, sgTitleArea.left, sgTitleArea.right, 0);
    }
...
int Gui_BindMenu(const unsigned char *psztitle, GUI_TEXT_ATTR stTitleAttr, GUI_TEXT_ATTR stTextAttr, const GUI_MENUITEM *pstMenuItem, GUI_MENU *pstMenu) {
    ...
    if (head)
        sprintf(pstMenu->szTitle, "%.*s", sizeof(pstMenu->szTitle), head);
    else
        memset(pstMenu->szTitle, 0, sizeof(pstMenu->szTitle));
    //pstMenu->stTitleAttr = stTitleAttr;
    pstMenu->stTitleAttr = gl_stTitleAttr;
...
int Gui_ShowMenuList(const GUI_MENU *pstMenu, enum MENUSTYLE eMode, int timeoutSec, int *piSelValue) {
    ...
    //stButtonAttr.eFontSize = GUI_FONT_SMALL;
    stButtonAttr.eFontSize = GUI_FONT_LARGE;
    ...
    iRet = GetMenuItem(pstMenu, eMode, sgMenuListArea, sgMenuButtons, numLines, &stButtonAttr, timeoutSec, piSelValue);
    sg_isCalled = 0;
    return iRet;
}
...
static void ClearScr(const Rect *rect, char IsClearBg) {
    ...
    if (!sg_bStartup && sg_isColorScreen) {
        ...
        ////////ShowLogs(1, "2. Width: %d", stLcdInfo.width);
        ////////ShowLogs(1, "2. Height: %d", stLcdInfo.height);
        ////////ShowLogs(1, "2. ppl: %d", stLcdInfo.ppl);
        ////////ShowLogs(1, "2. fgColor: %d", stLcdInfo.fgColor);
        ////////ShowLogs(1, "2. bgColor: %d", stLcdInfo.bgColor);
        if(!rect) {
...
static void UpdateCanvasSize() {
    if(sg_isColorScreen) {
        ...
        ////////ShowLogs(1, "1. Width: %d", stLcdInfo.width);
        ////////ShowLogs(1, "1. Height: %d", stLcdInfo.height);
        ////////ShowLogs(1, "1. ppl: %d", stLcdInfo.ppl);
        ////////ShowLogs(1, "1. fgColor: %d", stLcdInfo.fgColor);
        ////////ShowLogs(1, "1. bgColor: %d", stLcdInfo.bgColor);
    }
    else{
...
int getLeft(int nLeft, char *text)
{
    if(strncmp(text, "ECR MODE", 8) == 0)
        return 33;
    if(strncmp(text, "SENDING BACK RESPONSE", 21) == 0)
        return 2;
    ...
    {
        return 13;//Session key
    }else if(nLeft == 109)
    {
        return 14;//Master key
    }else if(nLeft == 100)
    {
        return 6;//Performing Callhome
    }else if(nLeft == 112)
    {
        return 15;
    }else if(nLeft == 115)
    ...
    {
        return 22;//param loaded
    }else if(nLeft == 139)
    {
        return 40;
    }else if(nLeft == 130)
    {
        //packing and successful
        return 41;//From 44
    }else if(nLeft == 120)
    {
        //pls enter pin
    ...
    }else if(nLeft == 142)
    {
        //pin ok
        return 32;
    }else if(nLeft == 249 && (strlen(text) < 6))
    {
        return 90;
    }
    else if(nLeft == 314)
    ...
    {
        return 0;
    }else if(nLeft == 141 && (strlen(text) < 7))
    {
        return 250;//Used to check cancel
    }else if(nLeft == 144 && (strlen(text) < 6))
    {
        return 250;//Used to enter during pin ok
    }else
        return 117;
}
...
int getTop(int nTop)
{
    ...
    {
        return 32;//Please enter pin
    }else if(nTop == 57)
    {
        return 25;//Amount in pin = 15
    }else if(nTop == 65)
    {
        return 35;//Please enter pin = 20
    }else if(nTop == 71)
    {
        return 25;
    }else if(nTop == 73)
    {
        return 30;//Pin unknown
    }else if(nTop == 89)
    {
        return 35;
    }else if(nTop == 93)
...
static void DrawText(const void *vRes, const void *vAttr, int top, int bottom, int left, int right, char isButton) {
    ...
    ////////ShowLogs(1, "Top: %d", nTop);
    ////////ShowLogs(1, "Left: %d", nLeft);

#ifdef _AR_MODE_
    ////////ShowLogs(1, "Inside DrawText 2a");
    if (sgFontM[stTextAttr.eFontSize].CharSet != CHARSET_ARABIA && AR_OPENFILE_ERROR == sg_uiArabicFileID) 
    {
#endif
        ////////ShowLogs(1, "Inside DrawText 2b");
        //ScrSelectFont(&sgFontS[stTextAttr.eFontSize], &sgFontM[stTextAttr.eFontSize]);
        ScrSelectFont(&sgFontS[3], &sgFontM[3]);
        //ScrFontSet(CFONT);
        ////////ShowLogs(1, "Inside DrawText 3");
        if (!sg_isColorScreen) 
        {
            ////////ShowLogs(1, "Inside DrawText 4a");
        ...
        else if(1)
        {
            ...
            if (stTextAttr.eStyle & GUI_FONT_REVERSAL) {
                //////ShowLogs(1, "1... left: %d, top: %d, right: %d, bottom: %d, sg_nDefBg: %d, sg_nDefColor: %d", left, top, right, bottom, sg_nDefBg, sg_nDefColor);
                if(sg_isColorScreen)
                {
                    if(checkBoard != -1)
                ...
                ////////ShowLogs(1, "2... left: %d, top: %d, right: %d, bottom: %d, sg_nDefBg: %d, sg_nDefColor: %d", left, top, right, bottom, sg_nDefBg, sg_nDefColor);
                //DrawBgBox(left, top, right, bottom, sg_nDefBg);
                //CLcdSetFgColor(sg_nDefColor);
            }
        }
        else{
            ////////ShowLogs(1, "Inside DrawText 4c");
            // Bug 689
            CLcdSetFgColor(sg_nDefColor);
        }
        ////////ShowLogs(1, "Inside DrawText 5");
        ...
        nTop += (int) (bottom - top - nLine * GetFontHeight(sgFontS[3], sgFontM[3])) / 2;
        if (nTop < top)
            nTop = top;
        ...
        do {
            ...
            if(!isButton){
                ...
                if((vFunc = GetCallbackEvent(GUI_CALLBACK_UPDATE_TEXT)))
                {
                    ...
                    ////////ShowLogs(1, "stText.y: %d", stText.y);
                    ////////ShowLogs(1, "nTop: %d", nTop);
                    if(stText.y != nTop){
                        nTop += stText.y;
                    }
                }
            ...
            width = GetStrPix((unsigned char *) szOneLine, &sgFontS[3], &sgFontM[3], 0);
            nLeft = left;
            ////////ShowLogs(1, "width: %d", width);
            ////////ShowLogs(1, "nLeft: %d", nLeft);
            ////////ShowLogs(1, "left: %d", left);
            ////////ShowLogs(1, "stTextAttr.eFontSize: %d", stTextAttr.eFontSize);

            switch (stTextAttr.eAlign) 
            {
                case GUI_ALIGN_RIGHT:
                    nLeft = right - width;
                    ////////ShowLogs(1, "Align: nLeft: %d", nLeft);
                    break;
                case GUI_ALIGN_CENTER:
                    nLeft += (right - left - width) / 2;
                    ////////ShowLogs(1, "Center: nLeft: %d", nLeft);
                    break;
            }

            if (nLeft < left)
                nLeft = left;
            ////////ShowLogs(1, "Outside: %d", nLeft);

            if (pStart != vRes) {
                //nTop += GetFontHeight(sgFontS[stTextAttr.eFontSize], sgFontM[stTextAttr.eFontSize]);
                nTop += GetFontHeight(sgFontS[3], sgFontM[3]);
                ////////ShowLogs(1, "Outside: nTop: %d", nTop);
                if (nTop >= bottom)
                    break;
            }
            ...
            if(!sg_isColorScreen) {
                if(noguide)
                {
                    if((strncmp(szOneLine, "CANCEL", 6) == 0) && (strlen(szOneLine) == 6))
                    {
                        break;
                    }else if((strncmp(szOneLine, "UP", 2) == 0) && (strlen(szOneLine) == 2))
                    {
                        break;
                    }else if((strncmp(szOneLine, "DN", 2) == 0) && (strlen(szOneLine) == 2))
                    {
                        break;
                    }else if((strncmp(szOneLine, "ENTER", 5) == 0) && (strlen(szOneLine) == 5))
                    {
                        break;
                    }else if((strlen(szOneLine) < 1))
                    {
                        ////////ShowLogs(1, "Text of: %s", szOneLine);
                        break;
                    }else
                    {
                        ////////ShowLogs(1, "Inside not colored 1...");
                        ////////ShowLogs(1, "Before... Text: %s. Length: %d", szOneLine, strlen(szOneLine));
                        //szOneLine[sg_nScrWidth/sgFontS[stTextAttr.eFontSize].Width] = 0;
                        ////////ShowLogs(1, "After... Text: %s. Length: %d", szOneLine, strlen(szOneLine));
                        ScrTextOut(nLeft, nTop, (unsigned char *)szOneLine);
                    }
                }else
                {
                    ////////ShowLogs(1, "Inside not colored 2...");
                    ////////ShowLogs(1, "Before... Text: %s. Length: %d", szOneLine, strlen(szOneLine));
                    //szOneLine[sg_nScrWidth/sgFontS[stTextAttr.eFontSize].Width] = 0;
                    ////////ShowLogs(1, "After... Text: %s. Length: %d", szOneLine, strlen(szOneLine));
                    ScrTextOut(nLeft, nTop, (unsigned char *)szOneLine);
                ...
                if(noguide)
                {
                    ...
                    {
                        //ShowLogs(1, "1. Left: %d --- Top: %d --- Text: %s", nLeft, nTop, szOneLine);
                        //CLcdTextOut(nLeft, nTop, (unsigned char *)szOneLine);
                        #ifndef REGULAR
                            if(clen)
                            {
                                y = llen;
                            ...
                            y = getTop(nTop);
                        #endif
                        x = getLeft(nLeft, (unsigned char *)szOneLine);
                        ScrTextOut(x, y, (unsigned char *)szOneLine);
                    }
                }else
                {
                    //ShowLogs(1, "2. Left: %d --- Top: %d --- Text: %s", nLeft, nTop, szOneLine);
                    //CLcdTextOut(nLeft, nTop, (unsigned char *)szOneLine);
                    #ifndef REGULAR
                        if(clen)
                        {
        ...
        if (sg_isColorScreen) {
            ////////ShowLogs(1, "Setting background color for colored");
            CLcdSetFgColor(sg_nDefColor);
        }
        else {
            ScrAttrSet(sg_nDefBg < 0xFF808080);
...
static void DrawRect(unsigned int left, unsigned int top, unsigned int right, unsigned int bottom, unsigned int color) {
    if (right >= (unsigned int) sg_nScrWidth)
        right = sg_nScrWidth - 1;
    //////ShowLogs(1, "DrawRect - Right: %d", right);
    if (bottom >= (unsigned int) sg_nScrHeight)
        bottom = sg_nScrHeight - 1;
    //////ShowLogs(1, "DrawRect - Right: %d", right);
    if (sg_isColorScreen) {
...
static int GetMenuItem(const GUI_MENU *pstMenu, enum MENUSTYLE eMode, Rect stMenuRect, const Rect *pstButtonRect, unsigned char nButtonNo, const GUI_TEXT_ATTR *pstButtonAttr, int timeoutSec, int *piSelected) {
    ...
    {
        //Title is ok
        //////ShowLogs(1, "Title: top - %d, bottom - %d, left - %d, right - %d", sgTitleArea.top, sgTitleArea.bottom, sgTitleArea.left, sgTitleArea.right);
    ...
    //////ShowLogs(1, "TotalLine: %d", nTotalLine);
    ...
    ClearScr(&pstButtonRect[0], 0);
    ClearScr(&pstButtonRect[1], 0);
    ClearScr(&pstButtonRect[2], 0);
    ClearScr(&pstButtonRect[3], 0);
    ...
    while (1) 
    {
        ...
        //Wisdom commented this out. Close comment in 2893
        //Issue is here
        while (1) 
        {
            //////ShowLogs(1, "Inside While 1");
            int iTouchStatus= 0;
        ...
        if (iKey == KEYENTER) {
                ...
                if (iCurScr > 0) 
                {
                    for (i = iCurScr; i > 0; i++) {
                        if (iHilightIndex >= ScrIndex[i - 1] && iHilightIndex < ScrIndex[i]) {
                            iCurScr = i - 1;
                            ////ShowLogs(1, "Step 2a - ii : %d -- %d", iCurScr, ScrIndex[i]);
                            break;
                        }
                    }
                }
                ////ShowLogs(1, "Step 2a - iii");
                ClearScr(&pstButtonRect[0], 0);
                ClearScr(&pstButtonRect[1], 0);
                ClearScr(&pstButtonRect[2], 0);
                ClearScr(&pstButtonRect[3], 0);
            ...
            for (i = iCurScr + 1; i < sizeof(ScrIndex) / sizeof(ScrIndex[0]); ++i) {
                ...
                if (iHilightIndex >= ScrIndex[i - 1] && iHilightIndex < ScrIndex[i]) {
                    ...
                    ////ShowLogs(1, "Step 3a - iib : %d -- %d", iCurScr, ScrIndex[i]);
                    break;
                }
            }
            ////ShowLogs(1, "Step 3a - iii");
            continue;
        }
    ...
    *piScrItemNum = 0;
    ...
    for (i = iCurItem; i < (int) pstMenu->nSize; ++i) {
        ...
        if (eMode & GUI_MENU_MANUAL_INDEX) {
            ...
            sprintf(buf, "%d.%s", i + 1, pstMenu->pstContent[i].szText);
            ////ShowLogs(1, "2.... %s", buf);
        }

        //////ShowLogs(1, "Text: %s", buf);
    ...
    if (pstItemsRect) 
    {
        ...
        while (i < iNumIndex) 
        {
            if ((i == iHilightLine)) 
            {
                ...
                //ClearScr(&stForClean);
                //////ShowLogs(1, "1. Left: %d", pstItemsRect[i].left);
                //////ShowLogs(1, "1. Top: %d", pstItemsRect[i].top);
                //////ShowLogs(1, "1. Right: %d", pstItemsRect[i].right);
                //////ShowLogs(1, "1. Bottom: %d", pstItemsRect[i].bottom);
                //////ShowLogs(1, "1. Text: %s", menu_text[i]);
            ...
            {
                //ClearScr(&stForClean);
                //////ShowLogs(1, "2. Left: %d", pstItemsRect[i].left);
                //////ShowLogs(1, "2. Top: %d", pstItemsRect[i].top);
                //////ShowLogs(1, "2. Right: %d", pstItemsRect[i].right);
                //////ShowLogs(1, "2. Bottom: %d", pstItemsRect[i].bottom);
                //////ShowLogs(1, "2. Text: %s", menu_text[i]);
                if(!sg_isColorScreen)
...
static void PrepareRes(){
    ...
    ////////ShowLogs(1, "sgTitleArea.bottom: %d", sgTitleArea.bottom);
    ////////ShowLogs(1, "sgTitleArea.right: %d", sgTitleArea.right);
    ...
    if(sg_hasTouchScreen && sg_nScrHeight > sg_nScrWidth){
        ...
        ////////ShowLogs(1, "It is not touchscreen");
        sgInputPromptArea.top = sgTitleArea.bottom;
